...
def resolve_submission_target(settings, force_queue=None):
    ...
    if re.match(pattern, url):
        ...
        try:
            ...
            show_message(
                "{}\nIt has to be an URL address or a list of URL addresses as JSON !".format(
                    url
                )
            )
            return

    urls = [url if "://" in url else "http://{}".format(url) for url in urls]
    ...
    if settings["COMFYUI_LOCAL"]:
        ...
        if re.match(pattern, comfyui_dir):
            ...
            try:
                ...
                show_message(
                    "{}\nIt must be a ComfyUI directory or a list of ComfyUI directories in JSON format!".format(
                        comfyui_dir
                    )
                )
                return
...
def interrupt(settings, client_id):
    ...
    def find_prompt_id(queue_list):
        return next(
            (r[1] for r in queue_list if r[3].get("client_id") == client_id), ""
        )
